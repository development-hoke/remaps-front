 ...
 */
class ZSubscriptionCrudController extends MasterController
{
    ...
    public function setup()
    {
        ...
        |--------------------------------------------------------------------------
        */
        $this->crud->setModel('App\Models\Zsubscription');
        $this->crud->setRoute('admin/zsubscription');
        $this->crud->setEntityNameStrings('zsubscription', 'zsubscription');
    ...
    public function showSubscriptionPackages(){
        if(!$this->user->hasActiveSubscription()){
            ...
            $data['packages'] = \App\Models\Zpackage::get();
     ...
     * @return \Illuminate\Http\Response
     */
    public function subscribeSubscription(Request $request, \App\Models\Zpackage $package){
        try {
            // get access token
    ...
    public function executeSubscription(Request $request){
        if ($request->has('success') && $request->query('success') == 'true') {
            ...
            try {
                // Execute agreement
                $subscription = new \App\Models\Zsubscription();
     ...
     * @return $response
     */
    public function cancelSubscription(\App\Models\Zsubscription $subscription){
        ...
        if($subscription->is_trial==1){
            ...
            try {
                $url = "https://api.sandbox.paypal.com/v1/billing/subscriptions/{$subscription->id}/cancel";
     ...
     * @return \Illuminate\Http\Response
     */
    public function immediateCancelSubscription(\App\Models\Zsubscription $subscription, Request $request){
        $user = \App\User::where("id",$subscription->user_id)->first();
        if($subscription->is_trial == 1){
            ...
            try {
                $url = "https://api.sandbox.paypal.com/v1/billing/subscriptions/{$subscription->id}/suspend";
